...
const GradientSvg = ({ className, isHovered, colors }) => {
  ...
  const currentSvgKey = svgOrder[animationFrame];
  const currentSvg = svgStates[currentSvgKey];
  const gradientId = `paint0_radial_${Math.random().toString(36).substr(2, 9)}`;
    ...
    >
      ...
      <defs>
        <radialGradient
          ...
          gradientTransform={currentSvg.gradientTransform}
        >
          {currentSvg.stops.map((stop, index) => (
            <stop
              key={index}
              offset={stop.offset}
    ...
    <>
      {Array.from({ length: 7 }).map((_, index) => (
        <div
          key={index}
          className={`absolute ${
            index < 3 ? 'w-[500px] h-[150px]' : 'w-[800px] h-[250px]'
          } ${
            index === 0
